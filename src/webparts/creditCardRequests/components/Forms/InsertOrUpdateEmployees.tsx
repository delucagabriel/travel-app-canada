...
const handleInsertOrUpdateEmployees = ()=>{
  ...
  .then(()=> GetAndCompareInformation.push(GetAllEmployees()
    .then(res => Employees = res))
  )
  .then(()=>{
      ...
      .then(()=> {
        ...
        .map((dado, index, arr ) => {
          ...
          if (employeesToUpdate.length === 100){
            callEmployeesServices.push(batchUpdateEmployees(employeesToUpdate).then(res => res));
            employeesToUpdate = [];
          }

          if (employeesToAdd.length === 100){
            callEmployeesServices.push(batchInsertEmployees(employeesToAdd).then(res => res));
            employeesToAdd = [];
          }
